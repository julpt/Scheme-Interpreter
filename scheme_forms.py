...
def do_define_form(expressions, env: Frame):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        eval_result = scheme_eval(expressions.rest.first, env)
        env.define(signature, eval_result)
        return signature
        # END PROBLEM 4
        ...
        env.define(symbol, lambda_function)
        return symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(val) is False:
            return False
...
def make_let_frame(bindings, env: Frame):
    ...
    while bindings is not nil:
        curr = bindings.first
        validate_form(curr, 2, 2)
        names = Pair(curr.first, names)
        validate_formals(names)
        vals = Pair(scheme_eval(curr.rest.first, env), vals)
